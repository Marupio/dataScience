...
std::cout << "schemaname.tablename = " << schemaName << "." << tableName << std::endl;
std::cout << "tableExists = " << tableExists << ", overwrite = " << overwrite << std::endl;
        if (!tableExists || overwrite) {
    ...
    std::vector<RiverRankPlayer> players(nPlayers);
    ...
    while (itP != players.end()) {
        ...
        randomizer << "makeRiverRankTables_" << tableI;
